...
from baxus_cma import EmbeddedTuRBO
from baxus_cma.benchmarks.benchmark_function import Benchmark
from baxus_cma.util.behaviors import BaxusBehavior
from baxus_cma.util.behaviors.gp_configuration import GPBehaviour
from baxus_cma.util.data_utils import join_data
from baxus_cma.util.projections import AxUS, ProjectionModel
from baxus_cma.util.utils import (from_1_around_origin,
...
class BAxUS(EmbeddedTuRBO):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            conf_name=conf_name,
            offset=offset,
            create_cand_cma=create_cand_cma,
        )
        ...
        self.extra_x_gp = None # previous data for building GP
        self.extra_x = None    # previous data for building GP
        self.extra_fx = None   # previous data for building GP
        self.prev_length = behavior.initial_base_length # previous baxus information
        self.prev_succcount = 0 # previous baxus information
        self.prev_failcount = 0 # previous baxus information
        self.terminated = False # has baxus restarted?
        self.offset = offset # for offset the printing
        self.baxus_data = [] # for logging
    ...
    def target_dim(self, target_dim: int) -> None:
        ...
        """
        self._dim_in_iterations[self.n_evals+self.offset] = target_dim
    ...
    def _adjust_length(self, fX_next) -> None:
        ...
        """
        debug(
            f"eval {self.n_evals+self.offset}: length = {self.length}, failcount = {self.failcount} (failtol = {self.failtol}), "
        ...
        if np.min(fX_next) < np.min(
            ...
            debug(f"eval {self.n_evals+self.offset}: increase success count")
            self.succcount += 1
            self.failcount = 0
        else:
            debug(f"eval {self.n_evals+self.offset}: increase failure count")
            self.succcount = 0
            self.failcount += 1
        if self.succcount == self.succtol:  # Expand trust region
            debug(f"eval {self.n_evals+self.offset}: expanding trust region")
            self.length = min([2.0 * self.length, self.length_max])
            self.succcount = 0
        elif self.failcount == self.failtol:  # Shrink trust region
            debug(f"eval {self.n_evals+self.offset}: shrinking trust region")
            self.length /= 2.0
            self.failcount = 0

        self._log_property("length_history", f"{self.n_evals+self.offset}:{self.length}")
    ...
    def _resample_and_restart(self, n_points: int, length: float = None) -> None:
        ...
        self._restart(length=self.prev_length)
        ...
        if self.n_init:
            ...
            self.baxus_data.append({
                'iter': self.offset+self.n_evals,
                'running_time': stamp2-stamp1,
                'tr': {
                ...
                'projection_matrix': {
                    'S': self.projector.S,
                    'target_dim': self.projector.target_dim,
                }
            })
            
        else:
            X_init = self.extra_x_gp
            X_init_up = self.extra_x
            fX_init = self.extra_fx
            self.succcount = self.prev_succcount
            self.failcount = self.prev_failcount
            n_pts = len(fX_init)
    ...
    def optimize(self) -> None:
        ...
        """
        self._log_property("projectors", f"{self.n_evals}:{self._projector_as_base64(self.projector)}")
        ...
        while self.n_evals < self.max_evals and not self._optimum_reached():
            ...
            if len(self._fX) > 1:
                ...
                info(f"{n_evals+self.offset}) Restarting with fbest = {fbest:.4}")
                ...
                if dims_and_bins:  # if we have a remaining-splitting dim
                    ...
                    self._split_points.append(self.n_evals+self.offset)
                    for splitting_dim, n_bins in dims_and_bins.items():
                        info(
                            f"eval {self.n_evals+self.offset}: splitting dimension {splitting_dim + 1} into {n_bins} new "
                    ...
                    self._dim_in_iterations[self.n_evals+self.offset] = self.target_dim
                    info(
                        f"eval {self.n_evals+self.offset}: new target dim = {self.target_dim}"
                    )
                    self._axus_change_iterations.append(self.n_evals+self.offset)
                    ...
                    warning(
                        f"eval {self.n_evals+self.offset}: cannot increase further. "
                        f"Re-starting with new HeSBO embedding and new TR."
                    )
        
                    self._axus_change_iterations.append(self.n_evals+self.offset)
                    self._trust_region_restarts.append(self.n_evals+self.offset)
                    self._dim_in_iterations[self.n_evals+self.offset] = self.target_dim          
                ...
                fbest = self._fX.min()
                info(f"eval {self.n_evals+self.offset}: starting from fbest = {fbest:.4}")
            ...
            while (
                ...
                X_next, X_next_up, fX_next, running_time = self._inner_optimization_step()
