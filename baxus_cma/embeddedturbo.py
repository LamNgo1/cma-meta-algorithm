...
from baxus_cma.benchmarks.other_methods import OptimizationMethod
from baxus_cma.gp import train_gp
from baxus_cma.util.acquisition_function_types import AcquisitionFunctionType
from baxus_cma.util.acquisition_functions import ExpectedImprovement
from baxus_cma.util.behaviors import EmbeddedTuRBOBehavior
from baxus_cma.util.behaviors.gp_configuration import GPBehaviour
from baxus_cma.util.projections import AxUS
from baxus_cma.util.space_learning.trust_region import create_Xcand
from baxus_cma.util.utils import (
...
class EmbeddedTuRBO(OptimizationMethod):
    ...
    def __init__(
        ...
        self.create_cand_cma=create_cand_cma  # TS candidates generation
    ...
    def _create_candidates(
        ...
        stamp3 = time.time()
        X_cand, lb, ub = self.create_cand_cma(
            n_cand=self.n_cand,
            S=self.projector.S,        
            length=length,
        )
        ...
        return (best_per_acq, hypers, lb, ub), (stamp2, stamp3, stamp4)
    ...
    def _inner_optimization_step(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        is_cands, stamps = self._create_candidates(
            X,
            fX,
            length=self.length,
        ...
        debug(f"eval {self.n_evals+self.offset} on {self.f.fun_name}: new point: {fX_next.min():.4} (current global / local best: {self.fX.min():.4}/{self._fX.min():.4})")
        
        if fX_next.min() < self.fX.min():
            n_evals, fbest = self.n_evals+self.offset, fX_next.min()
            info(f"eval {self.n_evals+self.offset} on {self.f.fun_name}: new best: {fbest:.4}")

        # Append data to the global history
        self.X = np.vstack((self.X, deepcopy(X_next_up)))
        self.fX = np.vstack((self.fX, deepcopy(fX_next)))
        stamp7 = time.time()
        stamp2, stamp3, stamp4 = stamps
        running_time = {
        ...
        return X_next, X_next_up, fX_next, running_time
