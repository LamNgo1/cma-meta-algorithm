...
def heuristic_bipedal(s, x):
    ...
    if state == STAY_ON_ONE_LEG:
        hip_targ[moving_leg] = x[0] # 1.1
        knee_targ[moving_leg] = x[1] #-0.6
        supporting_knee_angle += x[2] #0.03
        if s[2] > SPEED:
            supporting_knee_angle += x[2]
        supporting_knee_angle = min(supporting_knee_angle, SUPPORT_KNEE_ANGLE)
        knee_targ[supporting_leg] = supporting_knee_angle
        if s[supporting_s_base + 0] < x[3]: #0.10:  # supporting leg is behind
            state = PUT_OTHER_DOWN
    if state == PUT_OTHER_DOWN:
        hip_targ[moving_leg] = x[4] #+0.1
    ...
    if state == PUSH_OFF:
        ...
        if s[supporting_s_base + 2] > x[5] or s[2] > 10.0*x[6] * SPEED:
            state = STAY_ON_ONE_LEG
            moving_leg = 1 - moving_leg
            supporting_leg = 1 - moving_leg

    if hip_targ[0]:
        hip_todo[0] = x[7] * (hip_targ[0] - s[4]) - x[9] * s[5]
    if hip_targ[1]:
        hip_todo[1] = x[7] * (hip_targ[1] - s[9]) - x[9] * s[10]
    if knee_targ[0]:
        knee_todo[0] = x[8] * (knee_targ[0] - s[6]) - x[9] * s[7]
    if knee_targ[1]:
        knee_todo[1] = x[8] * (knee_targ[1] - s[11]) - x[9] * s[12]

    hip_todo[0] -= x[10] * (0 - s[0]) - 10.0*x[12] * s[1]  # PID to keep head strait
    hip_todo[1] -= x[10] * (0 - s[0]) - 10.0*x[12] * s[1]
    knee_todo[0] -= 100.0*x[11] * s[3]  # vertical speed, to damp oscillations
    knee_todo[1] -= 100.0*x[11] * s[3]
